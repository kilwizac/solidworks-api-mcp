---
type: method
interface: ISplitLineFeatureData
member: ISetSplitTargets
returns: void
parameters:
  -
    name: Count
    type: System.Int32
    description: Number of faces or bodies to split
  -
    name: DispArr
    type: System.Object
    description: in-process, unmanaged C++: Pointer to an array of faces or bodies to split VBA, VB.NET, C#, and C++/CLI: Not supported See In-process Methods for details about this type of method.
docset: sldworksapi
deprecated: false
since_version: unknown
categories:
  - features
related:
  - ISplitLineFeatureData.GetSplitTargetsCount
  - ISplitLineFeatureData.IGetSplitTargets
  - ISplitLineFeatureData.SplitAll
  - ISplitLineFeatureData.SplitTargets
keywords:
  - isetsplittargets
  - isplitlinefeaturedata
  - split
  - line
  - feature
  - data
  - targets
  - count
  - int32
  - disp
  - arr
  - object
  - void
---

# ISplitLineFeatureData.ISetSplitTargets

Sets the faces or bodies to split.

## Signature

```csharp
void ISetSplitTargets( 
   System.Int32
Count
,
   ref System.Object
DispArr
)
```
## Parameters

- `Count` (System.Int32): Number of faces or bodies to split
- `DispArr` (System.Object): in-process, unmanaged C++: Pointer to an array of faces or bodies to split VBA, VB.NET, C#, and C++/CLI: Not supported See In-process Methods for details about this type of method.

## Return Value

Unknown.

## Remarks

See
Accessing Selections that Define Features
for additional details.

## See Also

- `ISplitLineFeatureData.GetSplitTargetsCount`
- `ISplitLineFeatureData.IGetSplitTargets`
- `ISplitLineFeatureData.SplitAll`
- `ISplitLineFeatureData.SplitTargets`