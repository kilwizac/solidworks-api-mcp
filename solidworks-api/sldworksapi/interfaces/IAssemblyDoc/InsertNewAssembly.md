---
type: method
interface: IAssemblyDoc
member: InsertNewAssembly
returns: System.Int32
parameters:
  -
    name: FileName
    type: System.String
    description: Full pathname of new sub-assembly document ( see Remarks )
docset: sldworksapi
deprecated: false
since_version: unknown
categories:
  - assemblies
  - core
  - ui
related:
  - IAssemblyDoc.InsertNewVirtualAssembly
  - IComponent2.IsVirtual
  - IComponent2.SaveVirtualComponent
  - IModelDocExtension.IsVirtualComponent3
keywords:
  - virtual
  - assembly
  - assemblies
  - see
  - also
  - iassemblydoc
  - insertnewassembly
  - doc
  - insert
  - new
  - file
  - name
  - string
  - int32
  - save
  - vb
  - net
  - vba
---

# IAssemblyDoc.InsertNewAssembly

Creates a new virtual sub-assembly and optionally saves it to the specified file.

## Signature

```csharp
System.Int32 InsertNewAssembly( 
   System.String
FileName
)
```
## Parameters

- `FileName` (System.String): Full pathname of new sub-assembly document ( see Remarks )

## Return Value

Error code as defined by swInsertNewAssemblyErrorCode_e

## Remarks

If
Tools > Options > System Options > Assemblies > Save new components to external files
is:
selected, then this API creates a virtual sub-assembly and saves it to the file specified in FileName.
not selected, then this API ignores FileName, does not save the sub-assembly, and creates only a virtual sub-assembly in the FeatureManager design tree.

## Examples

- Insert and Save Virtual Assembly (C#) (Insert_and_Save_Virtual_Assembly_Example_CSharp.htm)
- Insert and Save Virtual Assembly (VB.NET) (Insert_and_Save_Virtual_Assembly_Example_VBNET.htm)
- Insert and Save Virtual Assembly (VBA) (Insert_and_Save_Virtual_Assembly_Example_VB.htm)

## See Also

- `IAssemblyDoc.InsertNewVirtualAssembly`
- `IComponent2.IsVirtual`
- `IComponent2.SaveVirtualComponent`
- `IModelDocExtension.IsVirtualComponent3`