...
parameters:
  ...
  -
    name: RevDir
    type: System.Boolean
    description: True to reverse the direction of the hole, false to not (see Remarks )
  -
    name: UseFeatScope
    type: System.Boolean
...
- `RevDir` (System.Boolean): True to reverse the direction of the hole, false to not (see Remarks )
...
The screw fit is defined as
swWzdHoleScrewClearanceTypes_e
and only used if a hole diameter is not specified; default value is NORMAL_FIT. The drill angle defaults to 118
,
if not specified. Input angles are in radians; input values are in meters.
...
For helicoil holes: Thread depth is calculated based on a constant (determined by hole type i.e. Insert = 1.0 * diameter) + thread depth from the standard * thread major diameter.
